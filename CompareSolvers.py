    ...
    for nAssortments in nAssortments_list:
        if sum(int(math.factorial(nProducts)/(math.factorial(nProducts-i))) for i in range(1, nProducts+1)) >= nAssortments:
            for seed in seed_list:
                # Generate instance
                instance_id += 1
                print('-'*10, 'Instance: %d with %d products and %d assortments - seed %d' %(instance_id, nProducts, nAssortments, seed), '-'*10)
                ...
                # Solve using Column Generation - Multiple Pricing = nPricing with optimal subset at new_col_penality
